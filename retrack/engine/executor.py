...
class RuleExecutor:
    ...
    def __get_input_params(
        self, node_dict: dict, current_node_filter: pd.Series, execution: Execution, include_context: bool = False, include_inputs: bool = False
    ) -> dict:
        input_params = {}
            ...
            for column in execution.payload.columns:
                ...
                if input_name not in input_params:
                    if current_node_filter is None:
                        ...
                        input_params[f"input_{column}"] = execution.payload.loc[current_node_filter, column]
    ...
    def __run_node(self, node_id: str, execution: Execution):
        ...
        include_context = node.kind() == NodeKind.CONNECTOR or node.kind() == NodeKind.FLOW
        include_inputs = node.kind() == NodeKind.FLOW
        input_params = self.__get_input_params(
            node.model_dump(by_alias=True), current_node_filter=current_node_filter, execution=execution, include_context=include_context, include_inputs=include_inputs
        )
