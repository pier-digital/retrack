...
class ComponentRegistry(Registry):
    ...
    It also provides indexes to access the data by class, kind and memory type."""
    ...
    def __register_in_keys_by_class_map(self, key: str, data: BaseNode) -> None:
    ...
    def __register_in_keys_by_kind_map(self, key: str, data: BaseNode) -> None:
        node_kind = data.kind()
    ...
    def __register_in_keys_by_memory_type_map(self, key: str, data: BaseNode) -> None:
    ...
    def __unregister_from_keys_by_class_map(self, key: str, data: BaseNode) -> None:
        class_name = data.__class__.__name__.lower()
        self._keys_by_class_map[class_name].remove(key)

    def __unregister_from_keys_by_kind_map(self, key: str, data: BaseNode) -> None:
        node_kind = data.kind()
        self._keys_by_kind_map[node_kind].remove(key)

    def __unregister_from_keys_by_memory_type_map(
        self, key: str, data: BaseNode
    ) -> None:
        memory_type = data.memory_type()
        self._keys_by_memory_type_map[memory_type].remove(key)

    def register(self, key: str, data: BaseNode, overwrite: bool = False) -> None:
        """Register an entry."""
        if not isinstance(data, BaseNode):
            raise ValueError("data must be a BaseNode instance.")

        super().register(key, data, overwrite=overwrite)

        self.__register_in_keys_by_class_map(key, data)
        self.__register_in_keys_by_kind_map(key, data)
        self.__register_in_keys_by_memory_type_map(key, data)
    ...
    def unregister(self, key: str) -> None:
        ...
        self.__unregister_from_keys_by_class_map(key, data)
        self.__unregister_from_keys_by_kind_map(key, data)
        self.__unregister_from_keys_by_memory_type_map(key, data)
