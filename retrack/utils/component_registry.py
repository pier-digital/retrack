...
class ComponentRegistry(Registry):
    ...
    def __register_in_indexes_by_name_map(self, name: str, data: BaseNode):
    ...
    def __register_in_indexes_by_kind_map(self, name: str, data: BaseNode):
        node_kind = data.kind()
    ...
    def __register_in_indexes_by_memory_type_map(self, name: str, data: BaseNode):
    ...
    def __unregister_from_indexes_by_name_map(self, name: str, data: BaseNode):
        node_name = data.__class__.__name__.lower()
        self._indexes_by_name_map[node_name].remove(name)

    def __unregister_from_indexes_by_kind_map(self, name: str, data: BaseNode):
        node_kind = data.kind()
        self._indexes_by_kind_map[node_kind].remove(name)

    def __unregister_from_indexes_by_memory_type_map(self, name: str, data: BaseNode):
        memory_type = data.memory_type()
        self._indexes_by_memory_type_map[memory_type].remove(name)

    def register(self, name: str, data: BaseNode, overwrite: bool = False):
        """Register an entry."""
    ...
    def _filter_connectors(self, connectors, connector_filter):
        result = []
    ...
    def get_node_input_connections(self, node_id: str, connector_filter=None):
    ...
    def get_node_output_connections(self, node_id: str, connector_filter=None):
        node_dict = self.get(node_id).model_dump(by_alias=True)
