...
def walk(parser, actual_id: str, skiped_ids=[]):
    element = parser.get_element_by_id(actual_id)
    skiped_ids.append(actual_id)
    # print(element.id, parser.get_name_by_id(element.id))
    ...
    for next_id in output_ids:
        if next_id not in skiped_ids:
            ...
            next_element_input_ids = get_element_connections(next_element, is_input=True)
            run_next = True
            ...
            if run_next:
                walk(parser, next_id, skiped_ids)
