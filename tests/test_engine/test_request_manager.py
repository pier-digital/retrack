...
def test_create_request_manager_with_dict(valid_input_dict_before_validation):
    with pytest.raises(TypeError):
        RequestManager(valid_input_dict_before_validation)


def test_create_request_manager_with_list_of_dicts(valid_input_dict_before_validation):
    pm = RequestManager([valid_input_dict_before_validation])
    assert len(pm.inputs) == 1
    assert pm.model is not None


def test_create_request_manager_with_list_of_dicts_and_duplicate_names(
...
def test_create_request_manager_with_invalid_input(valid_input_dict_before_validation):
    with pytest.raises(TypeError):
        RequestManager([valid_input_dict_before_validation, "invalid"])
